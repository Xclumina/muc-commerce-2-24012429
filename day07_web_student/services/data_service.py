...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    metrics = [
        ...
        {"label": "总体流失率", "value": f"{float(metric_map['流失率']) * 100:.1f}%", "note": "用户占比"},
    ...
    stage_name = max_churn["TenureGroup"]
    rate_pct = max_churn["流失率"] * 100
    insight = f"流失风险最高的生命周期阶段为「{stage_name}」，流失率达 {rate_pct:.2f}%，显著高于其他成熟阶段，是用户留存的核心攻坚窗口期。"
